...
class ProductContracts(unittest.TestCase):
    ...
    def test_operational_atlas_defaults_to_global_map_and_expands_business_instances(self) -> None:
        ...
        self.assertIn('labelCenterX: Number.isFinite(options.labelCenterX)', source)
        self.assertIn('const projectAnchors = [', source)
        self.assertIn('const point = atlasGlobePoint(globe, longitude, latitude)', source)
        self.assertIn('const leftLabels = globeScenarioNodes.slice', source)
        self.assertIn('node.labelCenterY = centerY - globeRadius * 0.94', source)
        ...
        self.assertIn('node.labelBounds = { left, right: left + width, top, bottom: top + height }', source)
        ...
        self.assertIn('默认以透明业务星球展示项目与场景', html)
        self.assertIn('按圆形关系展开真实动作', html)
