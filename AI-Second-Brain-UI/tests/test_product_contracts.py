...
class ProductContracts(unittest.TestCase):
    ...
    def test_operational_atlas_defaults_to_global_map_and_expands_business_instances(self) -> None:
        ...
        self.assertIn('labelPlacement: options.labelPlacement || "below"', source)
        self.assertIn('markerText: options.markerText || ""', source)
        ...
        self.assertIn('elements.atlasView.scrollIntoView({ behavior: "auto", block: "start" })', source)
        self.assertIn('function showAtlasOverview', source)
        self.assertIn('function drawAtlasFixedLabel', source)
        self.assertIn('const sectors = [', source)
        self.assertIn('const compactFocus = width < 480', source)
        self.assertIn('const compactOverview = width < 620', source)
        self.assertIn('const centerY = compactOverview ? Math.min(height * 0.39, 420)', source)
        self.assertIn('const globeRadius = Math.min', source)
        self.assertIn('function atlasGlobePoint', source)
        self.assertIn('globeDepth: Number.isFinite(options.globeDepth)', source)
        self.assertIn('globeSurface: Boolean(options.globeSurface)', source)
        self.assertIn('globeLongitude: Number.isFinite(options.globeLongitude)', source)
        self.assertIn('globeLatitude: Number.isFinite(options.globeLatitude)', source)
        self.assertIn('globeBaseRadius: Number(options.globeBaseRadius', source)
        self.assertIn('const projectAnchors = [', source)
        self.assertIn('const point = atlasGlobePoint(globe, longitude, latitude)', source)
        self.assertNotIn('const leftLabels = globeScenarioNodes.slice', source)
        self.assertNotIn('node.labelCenterY = centerY - globeRadius * 0.94', source)
        self.assertIn('function updateAtlasGlobeProjection', source)
        self.assertIn('function updateAtlasGlobeMotion', source)
        self.assertIn('const approachFactor = hoveredNode ? 0.12 : pointerInside ? 0.34 : 1', source)
        self.assertIn('function drawAtlasEdgeSignal', source)
        self.assertIn('function drawAtlasNodePulse', source)
        self.assertIn('canvas.dataset.activitySignals = String(activitySignals)', source)
        self.assertIn('const ambientActive = animate && Boolean(state.atlasGlobe || state.atlasFocusId)', source)
        self.assertIn('state.atlasTransitionAt = performance.now()', source)
        self.assertIn('function resolveAtlasGlobeLabels', source)
        self.assertIn('function drawAtlasGlobeLabel', source)
        self.assertIn('const labelDepthThreshold = canvasWidth < 420 ? 0.34 : canvasWidth < 620 ? 0.18 : 0.025', source)
        self.assertIn('Number(node.globeDepth || 0) > labelDepthThreshold', source)
        self.assertIn('canvas.dataset.labeledNodes = String(globeLabels.length)', source)
        self.assertIn('state.atlasGlobeRotation += elapsed * 0.000085', source)
        self.assertIn('state.atlasGlobeAutoUntil = performance.now() + 9000', source)
        self.assertIn('state.atlasGlobeAutoUntil = -1', source)
        self.assertIn('state.atlasDrag?.mode === "globe"', source)
        self.assertIn('state.atlasDrag.mode === "globe"', source)
        self.assertIn('state.atlasGlobeTilt = Math.max(-0.46', source)
        self.assertIn('function drawAtlasGlobe', source)
        self.assertIn('function drawAtlasGlobeGraticules', source)
        self.assertIn('context.createRadialGradient', source)
        self.assertIn('drawAtlasGlobeGraticules(context, globe, "back")', source)
        self.assertIn('drawAtlasGlobeGraticules(context, globe, "front")', source)
        self.assertIn('rgba(255, 255, 255, 0.52)', source)
        self.assertIn('rgba(8, 70, 70, 0.17)', source)
        self.assertIn('context.quadraticCurveTo(', source)
        self.assertNotIn('sphere.addColorStop(1, "#062b30")', source)
        self.assertIn('drawAtlasGlobe(context, state.atlasGlobe, now, pointerWorld)', source)
        self.assertIn('node.labelBounds = { left: label.left, right: label.right, top: label.top, bottom: label.bottom }', source)
        self.assertIn('return insideNode || insideLabel', source)
        self.assertNotIn('let cursorY = 112', source)
        self.assertNotIn('const laneWidth = width / Math.max(1, projects.length)', source)
        self.assertIn('{ kind: "action", start: 200, end: 335, label: "顺序动作"', source)
        self.assertIn('{ kind: "business-object", start: 340, end: 385, label: "业务对象"', source)
        self.assertIn('{ kind: "verification-gate", start: 40, end: 145, label: "验证门"', source)
        self.assertIn('sector.kind === "action" ? String(index + 1).padStart(2, "0")', source)
        self.assertIn('sourceEdges.forEach((edge) => {', source)
        self.assertIn('elements.knowledgeGraph.hidden = graph.nodes.length === 0', source)
        self.assertIn('drawAtlas(0)', source)
        self.assertNotIn('function renderAtlasFlow', source)
        self.assertNotIn('const scenarioClusters = new Map()', source)
        self.assertNotIn('`cluster:${scenario.id}:${kind}`', source)
        self.assertIn('业务世界总览会缓慢转动', html)
        self.assertIn('点击后展开真实动作', html)
        self.assertIn('id="atlasMotion"', html)
        self.assertIn('id="atlasGestureHint"', html)
        self.assertIn('tabindex="0" aria-label="动态知识关系图"', html)
        self.assertNotIn('id="atlasFlow"', html)
        self.assertIn('.atlas-project-card.is-muted', polish)
        self.assertIn('body .atlas-stage[data-atlas-mode="focus"]', polish)
        self.assertIn('body .atlas-stage[data-atlas-mode="overview"]', polish)
        self.assertIn('body .atlas-stage[data-atlas-mode="overview"] .atlas-orbit-one', polish)
        self.assertIn('@keyframes atlasAmbientDrift', polish)
        self.assertIn('@keyframes atlasOrbitOne', polish)
        self.assertIn('@media (prefers-reduced-motion: reduce)', polish)
        self.assertIn('.atlas-gesture-hint', polish)
        self.assertIn('.atlas-controls .atlas-motion', polish)
        self.assertIn('radial-gradient(circle at center', polish)
        self.assertNotIn('.atlas-flow-steps', polish)
